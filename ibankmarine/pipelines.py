...
class IibankmarinePipeline:

    # Database setup
    conn = sqlite3.connect('ibankmarine.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `ibankmarine`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM ibankmarine WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO ibankmarine (date, title, link, content)"
